...
> 技職盃黑客松 南區分區競賽 參賽專案｜SDGs 目標 12 — 負責任的消費與生產
...
`data.js` 中的廠商碳排數據並非虛構，而是依據以下來源人工整理：

- **主要參考**：`SDGs12資料來源.pdf`（附於本專案根目錄）
- **原始資料**：環境部氣候變遷署公開之企業溫室氣體排放量盤查報告
- **收錄範圍**：塑膠容器製造、飲料製造/包材、金屬容器、化學材料、印刷、紙製品、汽車貨運等行業，共 12 間廠商
- **數據內容**：各廠商之範疇一（直接排放）與範疇二（間接排放）數據，單位為公噸 CO₂e

> 本系統所有數據僅供 Hackathon 展示用途，若需正式採購決策請以官方最新公告為準。
